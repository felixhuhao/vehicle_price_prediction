"""
MLP 单独调参脚本 - log1p 目标变换版
  - 训练时使用 log1p(price) 作为目标
  - 预测时 expm1 还原
  - 对右偏分布更友好
"""
import pandas as pd
...
print(f"  n_mlp_per_fold={N_MLP_PER_FOLD}  target=log1p(price)")
...
def train_mlp(X_tr, y_tr, X_val, y_val, seed):
    ...
    np.log1p(y_val)  # validate log1p transform consistency

    X_tr_t = torch.FloatTensor(X_tr_s).to(device)
    y_tr_t = torch.FloatTensor(y_tr_log).to(device)
    ...
    criterion = nn.MSELoss()  # log 空间用 MSE 更合适
    ...
    return model, scaler, val_pred, best_val_mae, epoch + 1
...
submit.to_csv('mlp_log1p_submit.csv', index=False, encoding='utf-8')
print("已保存: mlp_log1p_submit.csv")
